...
app.post("/api/adblocker", (req, res) => {
    const { enabled } = req.body;
    
    if (enabled) {
        wisp.options.dns_method = "resolve";
        wisp.options.dns_servers = ["1.1.1.3", "1.0.0.3", "94.140.14.14"];
        wisp.options.dns_result_order = "ipv4first";
    }

    res.status(200).json({ message: 'Adblocker settings updated successfully' });
});
